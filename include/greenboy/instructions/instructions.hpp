...
template <typename T, typename U> struct enable_if_same : std::enable_if_t<std::is_same_v<T, U>> {};
...
struct is_readable<
    T, std::void_t<typename T::value_type,
                   enable_if_same<decltype(T::read),
                                  typename T::value_type (*)(CPU::RegisterSet &, MemoryBus &)>>>
    : std::true_type {};
...
struct is_writeable<
    T, std::void_t<typename T::value_type,
                   enable_if_same<decltype(T::write), void (*)(typename T::value_type,
...
template <Reg Name> struct Register {
  ...
  using value_type =
      decltype(read(std::declval<CPU::RegisterSet &>(), std::declval<MemoryBus &>()));
  static void write(value_type value, CPU::RegisterSet &registers, MemoryBus & /* memory */) {
    if constexpr (Name == Reg::B)
      registers.b = value;
    else if constexpr (Name == Reg::C)
      registers.c = value;
...
template <class Register, class ValueType = byte> struct Indirect {
  ...
  static constexpr cycles access_time() { return cycles{4} + Register::access_time(); }
};
...
template <class High, class Low> struct Double {
  ...
  static void write(word value, CPU::RegisterSet &registers, MemoryBus &memory) {
    High::write(high_byte(value), registers, memory);
    Low::write(low_byte(value), registers, memory);
  }
  static constexpr cycles access_time() { return High::access_time() + Low::access_time(); }
};

template <unsigned char Value> struct Constant {
  using value_type = byte;
  static byte read(CPU::RegisterSet &registers, MemoryBus &memory) { return byte{Value}; }
  static constexpr cycles access_time() { return cycles{0}; }
};
...
template <class Register, class Offset> struct Offsat {
  ...
  static constexpr cycles access_time() { return Register::access_time() + Offset::access_time(); }
};
...
inline cycles load(CPU::RegisterSet &registers, MemoryBus &memory) {
  ...
  static_assert(std::is_same_v<typename Source::value_type, typename Destination::value_type>);
...
template <typename LHS, typename RHS>
inline cycles subtract_with_carry(CPU::RegisterSet &registers, MemoryBus &memory) {
  static_assert(is_readable_v<LHS>);
  static_assert(is_readable_v<RHS>);
  static_assert(is_writeable_v<LHS>);
  static_assert(std::is_same_v<typename LHS::value_type, typename RHS::value_type>);
...
template <typename Register> cycles push(CPU::RegisterSet &registers, MemoryBus &memory) noexcept {
  static_assert(is_readable_v<Register>);
  static_assert(std::is_same_v<typename Register::value_type, word>);
...
template <typename Register> cycles pop(CPU::RegisterSet &registers, MemoryBus &memory) noexcept {
